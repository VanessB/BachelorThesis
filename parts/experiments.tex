...
решение, полученное с его помощью, сильно затухает и является некорректным.
...
\begin{figure}[ht!]
    ...
    \caption{Контуры $ 2 $-нормы невязки в зависимости от $ x \in [x^* - 15, x^* + 15] $ по горизонтали
        ...
        (где $ (x^*, y^*) $~--- среднее по ньютоновским итерациям)
        ...
        Черный путь соответствует итерациям Ньютона.
	}
...
\begin{sidewaysfigure}[!p]
    ...
    \begin{gnuplot}[terminal=epslatex, terminaloptions={color dashed size 7.5cm,6cm fontscale 0.6}]
        ...
        plot path.'implicit_euler.csv' every ::1 using 1:column t 'Неявный метод Эйлера' lw 2, \
             path.'bdf2.csv' every ::1 using 1:column t 'формула дифференцирования назад' lw 2, \
    ...
    \begin{gnuplot}[terminal=epslatex, terminaloptions={color dashed size 7.5cm,6cm fontscale 0.6}]
        ...
        plot path.'trapezoid.csv' every ::1 using 1:3 t 'метод трапеций' lw 2, \
             path.'L_pareschi_russo.csv' every ::1 using 1:3 t 'L-устойчивый метод Паресчи-Руссо' lw 2, \
             path.'reference.csv' every ::1 using 1:3 with lines t 'референсное решение' lw 3 lc 'black'
    \end{gnuplot}
    %
    \begin{gnuplot}[terminal=epslatex, terminaloptions={color dashed size 7.5cm,6cm fontscale 0.6}]
        ...
        plot path.'trapezoid.csv' every ::1 using 1:4 t 'метод трапеций' lw 2, \
             path.'L_pareschi_russo.csv' every ::1 using 1:4 t 'L-устойчивый метод Паресчи-Руссо' lw 2, \
             path.'reference.csv' every ::1 using 1:4 with lines t 'референсное решение' lw 3 lc 'black'
    \end{gnuplot}
    %
    \begin{gnuplot}[terminal=epslatex, terminaloptions={color dashed size 7.5cm,6cm fontscale 0.6}]
        ...
        plot path.'trapezoid.csv' every ::1 using 1:2 t 'метод трапеций' lw 2, \
             path.'L_pareschi_russo.csv' every ::1 using 1:2 t 'L-устойчивый метод Паресчи-Руссо' lw 2, \
    \end{gnuplot}
...
Неявный метод Эйлера, формула дифференцирования назад, а также метод трапеций на некоторой итерации сошлись к физически некорректному корню.
...
Взвешенный метод Эйлера также в этом смысле сравнительно затратен.
...
Данная система взята из \cite{bouchnita2020mathematical, vassilevski2020parallel} и является упрощённой моделью реального каскада свёртывания крови.
...
Применение неявного метода Эйлера напрямую ведёт к нефизичному отрицательному решению для $ \Delta t \geqslant 2 $.
